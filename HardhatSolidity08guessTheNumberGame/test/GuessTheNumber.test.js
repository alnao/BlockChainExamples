...
describe("GuessTheNumberMulti (NAO token)", function () {
	...
	it("più utenti possono avviare partite e aggiornarle", async function () {
		...
		expect(await c.getActiveSettersCount()).to.equal(2n);
	});

	it("guessAny indovina la partita giusta e paga il premio in NAO e rimuove il setter attivo", async function () {
		...
		const g1 = await c.games(setter1.address);
		expect(g1.active).to.equal(true);
		expect(await c.getActiveSettersCount()).to.equal(1n);
	});

	it("guessAny sbagliato divide la fee NAO tra tutti i prizePool attivi e admin (incluso recupero resto/dust)", async function () {
		const { c, nao, setter1, setter2, p1, setFee, guessFee } = await deployFixture();
		const n1 = 100000000000000000001n;
		const n2 = 100000000000000000002n;
		...
		const g1 = await c.games(setter1.address);
		const g2 = await c.games(setter2.address);
		expect(g1.prizePool).to.equal(setFee + guessFee / 2n / 2n);
		expect(g2.prizePool).to.equal(setFee + guessFee / 2n / 2n);
		expect(await c.adminBalance()).to.equal(guessFee - guessFee / 2n);
	});

	it("commit-reveal permette di indovinare in due fasi (anti-frontrunning)", async function () {
		const { c, nao, setter1, p1, setFee, guessFee } = await deployFixture();
		const secretNum = 100000000000000000001n;
		const salt = ethers.keccak256(ethers.toUtf8Bytes("my_salt"));
		
		await nao.connect(setter1).approve(c.target, setFee);
		await c.connect(setter1).startGame(secretNum);

		const commitmentHash = ethers.solidityPackedKeccak256(
			["address", "uint256", "bytes32"],
			[p1.address, secretNum, salt]
		);

		await nao.connect(p1).approve(c.target, guessFee);
		await expect(c.connect(p1).commitGuess(commitmentHash)).to.emit(c, "GuessCommitted");

		const before = await nao.balanceOf(p1.address);
		await expect(c.connect(p1).revealGuess(secretNum, salt)).to.emit(c, "Won");
		const after = await nao.balanceOf(p1.address);
		expect(after).to.equal(before + setFee + guessFee);
	});

	it("cancelGame cancella la partita dopo 30 giorni di inattività e restituisce il prizePool", async function () {
		const { c, nao, setter1, setFee } = await deployFixture();
		const n1 = 100000000000000000001n;
		await nao.connect(setter1).approve(c.target, setFee);
		await c.connect(setter1).startGame(n1);

		// Prima di 30 giorni fallisce
		await expect(c.connect(setter1).cancelGame()).to.be.revertedWithCustomError(c, "GameNotExpired");

		// Avanza il tempo di 30 giorni + 1 secondo
		await ethers.provider.send("evm_increaseTime", [30 * 24 * 3600 + 1]);
		await ethers.provider.send("evm_mine");

		const before = await nao.balanceOf(setter1.address);
		await expect(c.connect(setter1).cancelGame()).to.emit(c, "GameCancelled");
		const after = await nao.balanceOf(setter1.address);

		expect(after).to.equal(before + setFee);
		expect(await c.getActiveSettersCount()).to.equal(0n);
	});
	...
	it("applica il rate limit di 4 tentativi ogni 7 ore", async function () {
		...
		await expect(c.connect(p1).guessAny(wrong)).to.be.revertedWithCustomError(c, "TooManyAttempts");
